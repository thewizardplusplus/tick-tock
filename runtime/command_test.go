...
func TestCommandGroup_Run(test *testing.T) {
	for _, testData := range []struct {
		name         string
		makeCommands func(log *commandLog) CommandGroup
		wantLog      []int
		wantErr      assert.ErrorAssertionFunc
	}{
		{
			name:         "success without commands",
			makeCommands: func(log *commandLog) CommandGroup { return nil },
			wantErr:      assert.NoError,
		},
		{
			name: "success with commands",
			makeCommands: func(log *commandLog) CommandGroup {
				return newLoggableCommands(log, 5, withCalls())
			},
		...
		},
		{
			name: "error",
			makeCommands: func(log *commandLog) CommandGroup {
				return newLoggableCommands(log, 5, withErrOn(2))
			},
			wantLog: []int{0, 1, 2},
